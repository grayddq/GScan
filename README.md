...
## 程序特点 ##
    ...
    GScan      程序定位为安全人员提供的一项入侵检测工具，旨在尽可能的发现入侵痕迹，包括黑客遗留的病毒木马后门等文件、恶意的历史操作等不同的方式。
...
入侵检测的痕迹检测归纳为如下检测项，省去了一些安全配置和基线类的检测项。
注：对比内容会有部分片面，仅代表个人的观点归纳

| 检测项 | GScan | chkrootkit | rkhunter | lynis|
|:-------------|:------: |:------: |:------: |:------: |
| 版本 | v0.1 | v0.53 | v1.4.6 | v2.7.1 |
| 【检测前检查项】文件alias检查 | x | √ |  | x |
| 【主机文件检测】系统重要文件完整性检测 | x | √ | √ | x |
| 【主机文件检测】文件恶意特征扫描 | √ | x |  | x |
| 【主机文件检测】文件境外IP特征扫描 | √ | x |  | x |
| 【主机文件检测】敏感目录mount检测 | x | x |  | √ |
| 【主机操作检测】境外IP操作记录检测 | √ | x |  | x |
| 【主机操作检测】可疑操作或异常检测 | x | √ |  | x |
| 【主机进程检测】CPU&内存使用异常检测 | √ | x |  | √ |
| 【主机进程检测】I/O异常检测 | x | x |  | √ |
| 【主机进程检测】隐藏进程检测 | √ | x |  | x |
| 【主机进程检测】反弹shell进程检测 | √ | x |  | x |
| 【主机进程检测】可疑进程名称检测 | √ | x |  | x |
| 【主机进程检测】进程exe恶意特征检测 | √ | x |  | x |
| 【主机进程检测】僵尸进程检测 | x | x |  | √ |
| 【网络链接检测】境外IP链接检测 | √ | x |  | x |
| 【网络链接检测】恶意特征链接检测 | √ | √ |  | x |
| 【网络链接检测】网卡混杂模式检测 | √ | √ | √ | √ |
...
| 【常规后门检测】PROMPT_COMMAND后门检测 | √ | x |  | x |
| 【常规后门检测】Crontab后门检测 | √ | x |  | x |
| 【常规后门检测】alias后门检测 | √ | x |  | x |
| 【常规后门检测】SSH后门检测 | √ | x |  | x |
| 【常规后门检测】SSH Wrapper后门检测 | √ | x |  | x |
| 【常规后门检测】inetd.conf后门检测 | √ | x |  | x |
| 【常规后门检测】xinetd.conf后门检测 | √ | x |  | x |
| 【常规后门检测】系统启动项(/etc/init.d/)后门检测 | √ | x |  | √ |
| 【常规后门检测】系统启动项(/etc/rc.d/)后门检测 | √ | x |  | √ |
| 【常规后门检测】系统启动项(/etc/rc.local)后门检测 | √ | x |  | √ |
| 【常规后门检测】系统启动项(/usr/local/etc/rc.d')后门检测 | √ | x |  | √ |
| 【常规后门检测】系统启动项(/etc/conf.d/local.start)后门检测 | √ | x |  | √ |
| 【常规后门检测】系统启动项(/etc/inittab)后门检测 | √ | x |  | √ |
| 【账户安全检测】root权限账户检测 | √ | x |  | √ |
| 【账户安全检测】空口令账户检测 | √ | x |  | √ |
| 【账户安全检测】sudoers文件检测 | √ | x |  | √ |
| 【账户安全检测】用户组文件检测 | x | x |  | √ |
| 【账户安全检测】密码文件检测 | x | x |  | √ |
| 【日志安全检测】secure日志安全检测 | √ | x |  | x |
| 【日志安全检测】wtmp日志安全检测 | √ | √ |  | x |
| 【日志安全检测】utmp日志安全检测 | √ | √ |  | x |
| 【日志安全检测】lastlog日志安全检测 | √ | √ |  | x |
| 【安全配置检测】DNS设置检测 | √ | x |  | √ |
| 【安全配置检测】防火墙设置检测 | √ | x |  | √ |
| 【安全配置检测】/etc/hosts安全检测 | x | x |  | √ |
