...
| 【主机文件检测】文件恶意特征扫描 | √ |  |  | x |
| 【主机文件检测】文件境外IP特征扫描 | √ |  |  | x |
| 【主机文件检测】敏感目录mount检测 | x |  |  | √ |
| 【主机操作检测】境外IP操作记录检测 | √ |  |  | x |
| 【主机进程检测】CPU&内存使用异常检测 | √ |  |  | √ |
| 【主机进程检测】I/O异常检测 | x |  |  | √ |
| 【主机进程检测】隐藏进程检测 | √ |  |  | x |
| 【主机进程检测】反弹shell进程检测 | √ |  |  | x |
| 【主机进程检测】可疑进程名检测 | √ |  |  | x |
| 【主机进程检测】进程exe恶意特征检测 | √ |  |  | x |
| 【主机进程检测】僵尸进程检测 | x |  |  | √ |
| 【网络链接检测】境外IP链接检测 | √ |  |  | x |
| 【网络链接检测】恶意特征链接检测 | √ |  |  | x |
| 【网络链接检测】网卡混杂模式检测 | √ |  |  | √ |
| 【常规后门检测】LD_PRELOAD后门检测 | √ |  |  | x |
| 【常规后门检测】ld.so.preload后门检测 | √ |  |  | x |
| 【常规后门检测】PROMPT_COMMAND后门检测 | √ |  |  | x |
| 【常规后门检测】Crontab后门检测 | √ |  |  | x |
| 【常规后门检测】alias后门检测 | √ |  |  | x |
| 【常规后门检测】SSH后门检测 | √ |  |  | x |
| 【常规后门检测】SSH Wrapper后门检测 | √ |  |  | x |
| 【常规后门检测】inetd.conf后门检测 | √ |  |  | x |
| 【常规后门检测】xinetd.conf后门检测 | √ |  |  | x |
...
| 【账户安全检测】用户组文件检测 | x |  |  | √ |
| 【账户安全检测】密码文件检测 | x |  |  | √ |
| 【日志安全检测】secure日志安全检测 | √ |  |  | x |
| 【安全配置检测】DNS设置检测 | √ |  |  | √ |
| 【安全配置检测】防火墙设置检测 | √ |  |  | √ |
| 【安全配置检测】/etc/hosts安全检测 | x |  |  | √ |
| 【Rootkit检测】已知Rootkit类特征检测 | √ |  | √ | x |
